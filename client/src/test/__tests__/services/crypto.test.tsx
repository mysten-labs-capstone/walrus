...
describe("crypto service", () => {
  ...
  describe("encryptToBlob", () => {
    ...
    it("should include magic bytes in output", async () => {
      ...
      // Check for E2E_ENCRYPTED magic bytes
      const magic = new TextEncoder().encode("E2E_ENCRYPTED");
      // Compare as plain arrays to avoid subtle typed-array/Buffer equality issues
      expect(Array.from(bytes.slice(0, magic.length))).toEqual(
        Array.from(magic),
      );
    });
    ...
    it("should preserve file extension in metadata", async () => {
      ...
      const magic = new TextEncoder().encode("E2E_ENCRYPTED");
      const headerLen = new DataView(bytes.buffer).getUint32(
        magic.length,
        false,
      );
    ...
    it("should throw error for invalid private key", async () => {
      ...
      await expect(encryptToBlob(file, "invalid-key")).rejects.toThrow();
    });
  });
});
