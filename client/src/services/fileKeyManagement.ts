...
export async function deriveKEK(
  ...
  const salt = new TextEncoder().encode(
    SALT_PREFIX + accountMasterKeyHex.slice(0, 16),
  );
...
export async function wrapFileKey(
  ...
  const wrappedKeyBytes = await crypto.subtle.wrapKey("raw", fileKey, kek, {
    name: "AES-GCM",
    iv,
  });
 ...
 */
export async function importFileKeyFromShare(
  base64urlKey: string,
): Promise<CryptoKey> {
