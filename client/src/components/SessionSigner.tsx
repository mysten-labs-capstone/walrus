...
export default function SessionSigner() {
  ...
  const onSubmit = useCallback(
    (e: FormEvent) => {
      ...
      if (!isValidPrivateKey(norm))
        return setError("Expected a 32-byte hex private key (0x...).");
      setError(null);
      setPrivateKey(norm);
      setEditing(false);
    },
    ...
    <section className="space-y-4 rounded-2xl bg-white p-6 shadow-lg">
      <header className="flex items-center gap-3">
        <div>
          ...
          <p className="text-xs text-gray-500">
            Private key for file encryption (optional)
          </p>
        </div>
      </header>
      ...
      {editing ? (
        <form onSubmit={onSubmit} className="flex items-center gap-2">
          ...
          <button
            type="submit"
            className="rounded-lg bg-gradient-to-r from-cyan-600 to-blue-600 px-3 py-1.5 text-xs font-medium text-white hover:from-cyan-700 hover:to-blue-700"
          >
            Save
          </button>
          <button
        ...
        <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            {privateKey ? (
              ...
              <p className="text-sm text-gray-600">
                No encryption key set (files will not be encrypted)
              </p>
            )}
          </div>
          <div className="flex flex-wrap gap-3">
            ...
            >
              ...
              {privateKey ? "Change key" : "Set key"}
            </button>

            <button
              onClick={handleLogout}
              className="flex items-center gap-2 rounded-lg border border-red-100 bg-red-50 px-4 py-2 text-sm font-medium text-destructive hover:bg-destructive-20 hover:text-destructive dark:text-destructive-foreground"
            >
